...
internals.Model.prototype.validate_field = function(field_name, cb) {
    ...
    var parallelize = [];
    var ref = this.constructor.validations[field_name];
    for (var i = 0, il = ref.length; i < il; ++i) {
        var validation_function = ref[i];
...
internals.Model.prototype.set_many_to_one_association = function() {
    ...
    var ref = this.constructor.associations.many_to_one;
    var results = [];
    for (var i = 0, il = ref.length; i < il; ++i) {
        var association = ref[i];
...
internals.Model.prototype.set_many_to_many_association = function() {
    ...
    var ref = this.constructor.associations.many_to_many;
    var results = [];
    for (var i = 0, il = ref.length; i < il; ++i) {
        var association = ref[i];
...
internals.Model.prototype.build_many_to_many_function = function(association) {
    ...
    return function() {
        ...
        var model = internals.DataStorm.models[association.name];
        var join = {};
        ...
        var join_table = [this.constructor.table_name(), model.table_name()].sort().join('_');
        var dataset = model.dataset().select(model.table_name() + '.*').join(join_table, join).where(where);
        return dataset;
    };
};
